...
namespace TitleScene
{
    ...
    void Update() {
        if (GetAsyncKeyState(VK_UP) & 0x8000) {
            if (!isKeyLocked) { SoundManager::PlayScratch(); selectedBtn = (selectedBtn - 1 + BTN_COUNT) % BTN_COUNT;  isKeyLocked = true; }
        }
        else if (GetAsyncKeyState(VK_DOWN) & 0x8000) {
            if (!isKeyLocked) { SoundManager::PlayScratch(); selectedBtn = (selectedBtn + 1) % BTN_COUNT;isKeyLocked = true; }
        }
    ...
    void Draw() {
        ...
        const wchar_t* btnLabels[5] = { L"1. RELAY MODE", L"2. AUTO BATTLE", L"3. RANKINGS", L"4. SETTINGS", L"5. EXIT" };
