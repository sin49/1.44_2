...
#include <algorithm>
#include <gdiplus.h>
#include "SoundManager.h" // ⭐ 사운드 매니저 추가!
                                                             ...
                                                             void ProcessKey(int key) {
                                                                 if (key == VK_ESCAPE) {
                                                                     if (state == GameState::Title) {
                                                                         // PostQuitMessage(0); // ⭐ 프로그램 전체 강제 종료(강종) 주석 처리!
                                                                     }
                                                                     else {
                                                                         state = GameState::Title; // ⭐ 세팅창이나 인게임에서 ESC 누르면 타이틀로 이동!
                                                                     }
                                                                     return;
                                                                 }
                                                                 ...
                                                                 if (key == VK_SPACE && money >= 10 && state == GameState::Playing) {
                                                                     money -= 10; SpawnUnit(playerCenterX + (rand() % 200 - 100), playerCenterY + (rand() % 200 - 100), 0, (UnitType)(rand() % 4));
                                                                     SoundManager::PlayCoin(); // ⭐ 상점 구매 시 사운드!
                                                                 }
                                                                 ...
                                                                 if (mIdx >= 0 && mIdx < (int)mergeSuggestions.size()) {
                                                                     ...
                                                                     if (s.unitA != -1 && units[s.unitA].active && units[s.unitB].active && units[s.unitC].active) {
                                                                         ...
                                                                         SoundManager::PlayMidiNote(9, 49, 120, true);
                                                                         SoundManager::PlayScratch(); // ⭐ 합성 완료 시 사운드!
                                                                     }
                                                                 }
                                                             }
...
private:
    void UpdateLogic() {
        ...
        if (enemySpawnTimer <= 0) {
            enemySpawnTimer = max(0.8f, 4.0f - (gameTime / 120.f));
        ...
        for (int i = 0; i < MAX_UNITS; i++) {
            ...
            if (units[i].team == 2) {
                ...
                if (closestP != -1 && closestD < 640000.f) {
                    ...
                    if (closestD < 10000.f) { money += (int)units[i].damage; units[i].active = false; SoundManager::PlayMidiNote(9, 80, 60, false); }
                }
                    ...
                    while (j != -1 && checks < 500) {
                        if (i != j && units[j].active) {
                            ...
                            if (units[i].team == 4 && units[j].team != units[i].team && units[j].team < 2) {
                                ...
                                if (units[j].team == 3) { repD = cs * cs * 300.f; repF = 8000.f; }
                                else if (units[i].team == units[j].team) { repD = cs * cs * 350.f; repF = 12000.f; }
                                else { repD = cs * cs * 150.f; repF = 6000.f; }
            ...
            if (units[i].team != 4 && units[i].attackCooldown <= 0) {
                float slowAtk = 1.0f + ((float)rand() / RAND_MAX) * 0.8f;
    ...
    void HitUnit(int victim, int attacker, float dx, float dy, float dSq, float dmg) {
        ...
        SoundManager::PlayMidiNote(9, (rand() % 2 == 0) ? 76 : 77, 80, false);
    ...
    void RestartGame() {
        ...
        SoundManager::PlayFanfare_0(); // ⭐ 게임 시작 시 빰빠밤~
    }
    ...
    void Draw3DBox(Gdiplus::Graphics* g, float wx, float wy, float wz, float sx, float sy, float sz, float yaw, Gdiplus::Color c, float uiX = 0, float uiY = 0, float fscale = 1.f, bool flash = false) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (uiX == 0 && uiY == 0) { d[i] = (py - camY) * sp - (pz - camZ) * cp; p[i] = Project(px, py, pz); }
            else { d[i] = -pz; p[i].X = uiX + (px - wx) * fscale; p[i].Y = uiY - (py - wy) * fscale / 2.f - pz * fscale; }
        }
        Face faces[] = {
...
CAutoChessScene g_Game;

namespace GameB {
    HWND g_hWndB = NULL;
    ULONG_PTR gdiToken;
    bool gameoverchecker = false;
    static bool isGdiInit = false; // ⭐ GDI+ 중복 초기화 방지 플래그 추가

    void Init(HWND hWnd) {
        g_hWndB = hWnd;
        gameoverchecker = false;

        // ⭐ GDI+ 는 한 번만 초기화하여 메모리 누수 및 재진입 크래시 완벽 차단!
        if (!isGdiInit) {
    ...
    void Draw() {
        ...
        // ⭐ 텍스트 겹침 문제 해결: 안내 문구를 화면 맨 아래(rc.bottom - 40)로 내림!
        gr.DrawString(L"◆ AUTO BATTLE: Watch & Survive! (ESC to Quit) ◆", -1, &font, Gdiplus::PointF(15.0f, (float)rc.bottom - 40.0f), &brush);
    ...
    void Release() {
        g_Game.Release();
        // ⭐ GdiplusShutdown은 재진입을 위해 주석 처리하거나 빼는 것이 가장 안전합니다.
    }

    void InputKey(WPARAM wParam) {
        if (wParam == VK_ESCAPE && g_Game.state == GameState::Title) {
            gameoverchecker = true; // ⭐ 타이틀 화면에서 ESC를 한 번 더 누르면 프레임워크로 탈출!
        }
        g_Game.ProcessKey((int)wParam);
    }
    ...
    int GetScore() { return (int)g_Game.gameTime; }
}